...
async fn fetch_urls(urls: Vec<String>) -> Vec<Result<String, String>> {
    // buffer_unordered(5) 确保最多 5 个 futures 同时被轮询
    // 不需要单独的 Semaphore。
    let results: Vec<_> = stream::iter(urls)
        ...
        .buffer_unordered(5) // ← 仅这个就将并发限制为 5
        .collect()
        .await;
...
// 注意：当您需要在独立生成的任务（tokio::spawn）之间限制并发时使用 Semaphore。
// 当处理流时使用 buffer_unordered。不要为同一个限制组合两者。
```
...
### 练习 3：带 Worker Pool 的优雅关闭

构建一个任务处理器，具有：
- 基于通道的工作队列
- N 个 worker 任务从队列消费
- Ctrl+C 时优雅关闭：停止接受、完成处理中的工作
...
### 练习 4：从零开始构建简单的异步 Mutex

使用通道实现一个异步感知的 mutex（不使用 `tokio::sync::Mutex`）。
...
pub struct SimpleGuard<T> {
    data: Arc<UnsafeCell<T>>,
    _permit: OwnedSemaphorePermit, // 在 guard drop 时 drop → 释放锁
}
...
impl<T> std::ops::Deref for SimpleGuard<T> {
    type Target = T;
    fn deref(&self) -> &T {
        // SAFETY：我们持有唯一的信号量许可，所以不存在其他
        // SimpleGuard → 独占访问是有保证的。
...
impl<T> std::ops::DerefMut for SimpleGuard<T> {
    fn deref_mut(&mut self) -> &mut T {
        // SAFETY：同样的推理 — 单个许可保证独占性。
        unsafe { &mut *self.data.get() }
    }
}

// 当 SimpleGuard 被 drop 时，_permit 被 drop，
// 这释放了信号量许可 — 另一个 lock() 可以继续。
...
**关键要点**：异步 mutex 通常构建在信号量之上。信号量提供异步等待机制 — 当被锁定时，`acquire()` 挂起任务直到许可被释放。这正是 `tokio::sync::Mutex` 内部的工作方式。

> **为什么是 `UnsafeCell` 而不是 `std::sync::Mutex`？** 这个练习的早期版本使用了 `Arc<Mutex<T>>`，`Deref`/`DerefMut` 调用 `.lock().unwrap()`。
> 那不能编译 — 返回的 `&T` 借用了一个立即被 drop 的临时 `MutexGuard`。
> `UnsafeCell` 避免了中间 guard，基于信号量的序列化使 `unsafe` 变得合理。
...
不使用 `tokio::select!` 或 `tokio::time::timeout`，实现一个函数，让 future 与截止日期竞速，超时时返回 `Either::Left(result)` 或 `Either::Right(())`。

*提示*：基于第 6 章的 `Select` 组合器和同一章的 `TimerFuture` 构建。
...
**关键要点**：`select`/`timeout` 只是轮询两个 futures 看看哪个先完成。整个异步生态系统从这个简单的原语构建：轮询、Pending/Ready、Waker。
