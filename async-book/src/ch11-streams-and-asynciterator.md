# 11. Streams 和 AsyncIterator 🟡

> **你将学到：**
> - `Stream` trait：多个值的异步迭代
> - 创建 streams：`stream::iter`、`async_stream`、`unfold`
> - Stream 组合器：`map`、`filter`、`buffer_unordered`、`fold`
> - 异步 I/O traits：`AsyncRead`、`AsyncWrite`、`AsyncBufRead`

## Stream Trait 概述

`Stream` 对于 `Iterator` 的关系，如同 `Future` 对于单个值的关系——它异步地产出多个值：
...
// 2. 从异步生成器（使用 async_stream crate）
...
// 4. 从通道接收器（tokio_stream::wrappers）
...
let s = stream::unfold(0u32, |state| async move {
    if state >= 5 {
        ...
        let next = state + 1;
        Some((state, next)) // 产出 `state`，新状态是 `next`
    }
});
```
...
async fn stream_examples() {
    ...
    // take, skip, zip, chain — 就像 Iterator 一样
...
| **取消** | Drop stream | `CancellationToken` |
| **背压** | 消费者控制轮询速率 | 消费者控制 `MoveNextAsync` |
| **内置** | 否（需要 `futures` crate） | 是（自 C# 8.0） |
| **组合器** | `.map()`、`.filter()`、`.buffer_unordered()` | LINQ + `System.Linq.Async` |
| **错误处理** | `Stream<Item = Result<T, E>>` | 在异步迭代器中抛出 |

```rust
// Rust：数据库行的 Stream
// 注意：在体内使用 ? 时需要 try_stream! 而不是 stream!。
...
**挑战**：给定一个传感器读数的 stream `Stream<Item = f64>`，编写一个异步函数消费该 stream 并返回 `(count, min, max, average)`。使用 `StreamExt` 组合器——不要只是收集到 Vec 中。
...
**关键要点**：像 `.fold()` 这样的 Stream 组合器一次处理一个项而不收集到内存中——对于处理大型或无界数据流至关重要。
...
正如 `std::io::Read`/`Write` 是同步 I/O 的基础一样，它们的异步版本是异步 I/O 的基础。这些 traits 由 `tokio::io` 提供（或 `futures::io` 用于运行时无关代码）：

```rust
// tokio::io — std::io traits 的异步版本
...
**实际上**，你很少直接调用这些 `poll_*` 方法。相反，使用扩展 traits `AsyncReadExt` 和 `AsyncWriteExt`，它们提供 `.await` 友好的辅助方法：
...
> **tokio vs futures I/O traits**：它们相似但不完全相同——tokio 的 `AsyncRead` 使用 `ReadBuf`（安全处理未初始化内存），而 `futures::AsyncRead` 使用 `&mut [u8]`。使用 `tokio_util::compat` 在两者之间转换。

> **拷贝工具**：`tokio::io::copy(&mut reader, &mut writer)` 是 `std::io::copy` 的异步等价物——对代理服务器或文件传输有用。`tokio::io::copy_bidirectional` 同时拷贝两个方向。
...
**关键要点**：通过针对 `AsyncBufRead` 而不是具体类型编程，你的 I/O 代码可以跨文件、socket、管道甚至内存缓冲区（`tokio::io::BufReader::new(std::io::Cursor::new(data))`）重用。

</details>
</details>

> **核心要点 — Streams 和 AsyncIterator**
> - `Stream` 是 `Iterator` 的异步等价物——产出 `Poll::Ready(Some(item))` 或 `Poll::Ready(None)`
> - `.buffer_unordered(N)` 并发处理 N 个 stream 项——这是 streams 的关键并发工具
> - `async_stream::stream!` 是创建自定义 streams 最简单的方式（使用 `yield`）
> - `AsyncRead`/`AsyncBufRead` 实现跨文件、socket 和管道的通用、可重用 I/O 代码

> **另见：** [第 9 章 — 何时 Tokio 不是最佳选择](ch09-when-tokio-isnt-the-right-fit.md) 了解 `FuturesUnordered`（相关模式），[第 13 章 — 生产模式](ch13-production-patterns.md) 了解有界通道的背压
